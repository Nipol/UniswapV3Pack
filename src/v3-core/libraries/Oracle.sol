...
library Oracle {
    ...
    function transform(
        ...
        int56 x;
        bytes4 bdelta = bytes4(delta);
        assembly {
            x := mload(add(bdelta, 0x20))
        }
        return
            Observation({
                blockTimestamp: blockTimestamp,
                tickCumulative: last.tickCumulative + int56(tick) * x,
        ...
        )
    {
        ...
        if (target == beforeOrAt.blockTimestamp) {
            ...
            int56 obTimeDelta;
            bytes4 bdelta = bytes4(observationTimeDelta);
            assembly {
                obTimeDelta := mload(add(bdelta, 0x20))
            }
            uint32 targetDelta = target - beforeOrAt.blockTimestamp;
            int56 tarDelta;
            bdelta = bytes4(targetDelta);
            assembly {
                tarDelta := mload(add(bdelta, 0x20))
            }
            return (
                beforeOrAt.tickCumulative +
